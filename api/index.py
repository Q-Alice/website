...
def compile_and_run():
    ...
    try:
        ...
        qc = validate_qasm(file_path)
        if not qc:
            return jsonify({"error": f"Invalid QASM file: {filename}"}), 400
    except Exception as e:
...
def validate_qasm(file_path):
    try:
        ...
        print(f"Error validating QASM file {file_path}: {e}")
        return None 
